from modules.TCGN.CMT_block import *
from modules.TCGN.gnn_block import Graph_Encoding_Block_big,Graph_Encoding_Block
from modules.TCGN.transformer_block import Channel_Attention
...
class TCGNModel(pl.LightningModule):
    def __init__(self, img_size=224, in_chans=3, n_genes=785, embed_dims=[46, 92, 184, 368], stem_channel=16,
                 fc_dim=1280,
                 ...
                 learning_rate=1e-5, patch_level = False):
        super().__init__()
        ...
        self.head = nn.Sequential(nn.Linear(fc_dim, fc_dim*4, bias=True),nn.GELU(),nn.Linear(fc_dim*4,n_genes, bias=True))
        # nn.Linear(fc_dim, n_genes, bias=True) if n_genes > 0 else nn.Identity()
        # nn.Sequential(nn.Linear(fc_dim, fc_dim*4, bias=True),nn.Linear(fc_dim*4,n_genes))
    ...
    def reset_classifier(self, n_genes, global_pool=''):
        self.n_genes = n_genes
        self.head = nn.Linear(self.fc_dim, n_genes, bias=True) if n_genes > 0 else nn.Identity()
    ...
    def configure_optimizers(self):
        ...
        gnn_params = list(map(id, self.gnn_block0.parameters()))
        base_params = filter(lambda p: id(p) not in gnn_params, self.parameters())
        optimizer = torch.optim.Adam([{'params': base_params},
                                    {'params': self.gnn_block0.parameters(), 'lr': 2.5e-4}],
                                    lr=self.learning_rate, betas=(0.9, 0.999), eps=1e-08, weight_decay=0,amsgrad=False)
